  ...
  while (fgets(line, sizeof(line), file) && keymap_count < BUTTON_COUNT) {
    ...
    if (sscanf(line, "%d,%d=%19s", &row, &col, keycode) == 3) {
      if (row >= 0 && row < BUTTON_ROWS && col >= 0 && col < BUTTON_COLS) {
        ...
        strncpy(keymap[keymap_count].keycode, keycode,
                sizeof(keymap[keymap_count].keycode) - 1);
...
static uint32_t keycode_string_to_lvgl_key(const char *keycode) {
  // Navigation keys
  if (strcasecmp(keycode, "UP") == 0)
    return LV_KEY_UP;
  if (strcasecmp(keycode, "DOWN") == 0)
    return LV_KEY_DOWN;
  if (strcasecmp(keycode, "LEFT") == 0)
    return LV_KEY_LEFT;
  if (strcasecmp(keycode, "RIGHT") == 0)
    return LV_KEY_RIGHT;
  if (strcasecmp(keycode, "RETURN") == 0 || strcasecmp(keycode, "ENTER") == 0)
    return LV_KEY_ENTER;
  if (strcasecmp(keycode, "BACKSPACE") == 0)
    return LV_KEY_BACKSPACE;
  if (strcasecmp(keycode, "DELETE") == 0)
    return LV_KEY_ESC; // AC button
  ...
  if (strcasecmp(keycode, "LSHIFT") == 0)
    return 'S'; // Shift key
  if (strcasecmp(keycode, "LALT") == 0)
    return 'A'; // Alpha key
  if (strcasecmp(keycode, "PAGEUP") == 0)
    return 'M'; // Menu key
  if (strcasecmp(keycode, "PAGEDOWN") == 0)
    return 'O'; // On key

  // Function keys mapped to calculator functions
  if (strcasecmp(keycode, "F1") == 0)
    return 'G'; // CALC button (graph toggle)
  if (strcasecmp(keycode, "F2") == 0)
    return 'W'; // x^2 button
  if (strcasecmp(keycode, "F3") == 0)
    return 'X'; // ^ (power) button
  if (strcasecmp(keycode, "F4") == 0)
    return 'L'; // log button
  if (strcasecmp(keycode, "F5") == 0)
    return 'V'; // VAR/x button
  if (strcasecmp(keycode, "F6") == 0)
    return 'K'; // CONST button
  if (strcasecmp(keycode, "F7") == 0)
    return 'Q'; // EXP button
  if (strcasecmp(keycode, "F8") == 0)
    return 'f'; // FRAC button
  if (strcasecmp(keycode, "F9") == 0)
    return 'r'; // sqrt button
  if (strcasecmp(keycode, "F10") == 0)
    return 'l'; // ln button
  if (strcasecmp(keycode, "F11") == 0)
    return 's'; // sin button
  if (strcasecmp(keycode, "F12") == 0)
    return 'c'; // cos button
  if (strcasecmp(keycode, "TAB") == 0)
    return 't'; // tan button

  // X variable key (backtick on keyboard -> 'V' for VAR/x insert)
  if (keycode[0] == '`')
    return 'V';

  // Single character keys - return as-is (preserve case)
  if (strlen(keycode) == 1) {
    return (uint32_t)keycode[0];
  }

  // For multi-char like "SPACE", return first char
  if (strcasecmp(keycode, "SPACE") == 0)
    return ' ';
...
static void button_flash_timer_cb(lv_timer_t *timer) {
  ...
  lv_obj_set_style_bg_color(btn, lv_color_hex(0x404040),
...
static void create_button_panel(lv_obj_t *parent) {
  ...
  for (int r = 0; r < BUTTON_ROWS; r++) {
    for (int c = 0; c < BUTTON_COLS; c++) {
      ...
      // Get label from button_keymap first
      const char *display_label = button_keymap_get_label(r, c);
      if (display_label) {
      ...
      for (int i = 0; i < keymap_count; i++) {
        if (keymap[i].row == r && keymap[i].col == c) {
          ...
          // Check if this is an arrow key and adjust position for diamond
          // layout
          if (strcasecmp(keymap[i].keycode, "UP") == 0) {
            // UP key - shift half-position up from row 0
            lv_obj_set_size(btn, btn_width * (2.0 / 3.0), btn_height);
            pos_x -= (btn_width + 1.5 * margin) / 3.0 - 1;
            is_arrow = true;
          } else if (strcasecmp(keymap[i].keycode, "DOWN") == 0) {
            // DOWN key - shift half-position down from row 1
            lv_obj_set_size(btn, btn_width * (2.0 / 3.0), btn_height);
            pos_x += 2 * (btn_width + 1.5 * margin) / 3.0;
            is_arrow = true;
      ...
      lv_obj_set_style_bg_color(btn, lv_color_hex(0x404040),
                                LV_PART_MAIN | LV_STATE_DEFAULT);
      lv_obj_set_style_border_color(btn, lv_color_hex(0x606060),
                                    LV_PART_MAIN | LV_STATE_DEFAULT);
...
int main(int argc, char **argv) {
  ...
  // Position the main display window
  SDL_Window *main_window = lv_sdl_window_get_window(main_disp);
  SDL_SetWindowPosition(main_window, 100, 100);
  SDL_SetWindowTitle(main_window, "Calculator Display");

  // Create second SDL window for button panel
  lv_display_t *button_disp = lv_sdl_window_create(320, 270);

  // Position the button panel window below the main display
  SDL_Window *button_window = lv_sdl_window_get_window(button_disp);
  SDL_SetWindowPosition(button_window, 100, 100 + 240 + 30); // 30px gap between windows
  SDL_SetWindowTitle(button_window, "Calculator Keypad");
  ...
  // Initialize button keymap for labels
  button_keymap_init();

  // Create button panel on second display
  lv_display_set_default(button_disp);
  lv_obj_t *button_screen = lv_screen_active();
  lv_obj_set_style_bg_color(button_screen, lv_color_hex(0x202020),
                            LV_PART_MAIN | LV_STATE_DEFAULT);
  lv_obj_set_scrollbar_mode(button_screen, LV_SCROLLBAR_MODE_OFF);
  lv_obj_clear_flag(button_screen, LV_OBJ_FLAG_SCROLLABLE);
