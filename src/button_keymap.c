...
static void load_default_keymap(void) {
  ...
  // Row 2: var | calc | v | > | const | EXP
  keymap[keymap_count++] = (button_mapping_t){1, 0, 'V', "VAR"};
  keymap[keymap_count++] = (button_mapping_t){1, 1, 'G', "CALC"};   // Changed from 'C' to 'G' (Graph)
  ...
  // Row 3: frac | sqrt | x^2 | ^x | log | ln
  keymap[keymap_count++] = (button_mapping_t){2, 0, 'f', "FRAC"};
  keymap[keymap_count++] = (button_mapping_t){2, 1, 'r', "sqrt"};
  keymap[keymap_count++] = (button_mapping_t){2, 2, 'W', "x^2"};   // Changed from '2' to 'W' (special: insert "^2")
  keymap[keymap_count++] = (button_mapping_t){2, 3, 'X', "^"};     // Changed from '^' to 'X' (special: insert "^")
  ...
  keymap[keymap_count++] = (button_mapping_t){3, 4, 'c', "cos"};   // Changed from 'C' to 'c'
  keymap[keymap_count++] = (button_mapping_t){3, 5, 't', "tan"};

  // Row 5: A | B | C | D | E | F  (unused in most apps)
  ...
  // Row 9: e | 0 | . | pi | Ans | =
  keymap[keymap_count++] = (button_mapping_t){8, 0, '$', "e"};
  keymap[keymap_count++] = (button_mapping_t){8, 1, '0', "0"};
  keymap[keymap_count++] = (button_mapping_t){8, 2, '.', "."};
  keymap[keymap_count++] = (button_mapping_t){8, 3, 'P', "\xCF\x80"};  // UTF-8 π
