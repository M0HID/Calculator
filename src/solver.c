...
static int newton_raphson(const char *expr, double x0, double *result, int *iterations) {
  double x = x0;
  for (int i = 0; i < MAX_ITERATIONS; i++) {
    double fx = eval_expression_x(expr, x);
    if (fabs(fx) < TOLERANCE) { *result = x; *iterations = i+1; return 1; }
    double fpx = numerical_derivative(expr, x, 1e-6);
    if (fabs(fpx) < 1e-12) return 0;
    x = x - fx / fpx;
    if (i > 0 && fabs(fx) < TOLERANCE) { *result = x; *iterations = i+1; return 1; }
  }
  *result = x; *iterations = MAX_ITERATIONS; return 0;
}
...
static int solve_quadratic(double a, double b, double c, double *x1, double *x2) {
  if (fabs(a) < 1e-12) return solve_linear(b, c, x1) ? 1 : 0;
  double d = b*b - 4*a*c;
  if (d < 0) return 0;
  if (fabs(d) < 1e-12) { *x1 = -b/(2*a); *x2 = *x1; return 1; }
  double sd = sqrt(d);
  *x1 = (-b+sd)/(2*a); *x2 = (-b-sd)/(2*a); return 2;
}
...
/* Create a standard solver screen */
static lv_obj_t *setup_solver_screen(const char *hint_text) {
  ...
  if (indev) lv_indev_set_group(indev, solver_group);
...
static const SubMenuItem solver_menu_items[] = {
  ...
  {"Curve Fitting", NULL},  /* Placeholder, callback set below */
  {"Function Storage", NULL},  /* Placeholder, callback set below */
};

#define SOLVER_MENU_ITEMS 5

/* Forward declare placeholder callback wrappers */
static void open_curve_fitting(void) { show_placeholder("Curve Fitting"); }
static void open_function_storage(void) { show_placeholder("Function Storage"); }

static void show_solver_menu(void) {
  current_mode = SOLVER_MENU;
  cleanup_solver_ui();

  /* Create mutable copy for placeholders */
  ...
  solver_group = ui_create_submenu(menu_items_copy, SOLVER_MENU_ITEMS, &style, main_menu_create);
}

/* ── Generic sub-solver key handler (ESC=back, M=menu, Enter=solve) ── */
...
/* Key handler on individual textareas for sub-solvers: Up/Down navigate, Enter solves */
...
static lv_obj_t *linear_fields[3]; /* a, b, result */
...
static void solve_linear_fn(lv_obj_t **f, int n, lv_obj_t *res) {
  ...
  double x;
  char buf[128];
  if (solve_linear(a, b, &x)) snprintf(buf, sizeof(buf), "x = %.6g", x);
  else snprintf(buf, sizeof(buf), "No solution (a=0)");
...
static lv_obj_t *quad_fields[4]; /* a, b, c, result */
...
static void solve_quad_fn(lv_obj_t **f, int n, lv_obj_t *res) {
  ...
  if (ns == 0)      snprintf(buf, sizeof(buf), "No real solutions");
  else if (ns == 1)  snprintf(buf, sizeof(buf), "x = %.6g", x1);
  else               snprintf(buf, sizeof(buf), "x1 = %.6g\nx2 = %.6g", x1, x2);
  lv_label_set_text(res, buf);
}
...
static lv_obj_t *nr_fields[3]; /* expr, x0, result */
...
static void placeholder_key_cb(lv_event_t *e) {
  uint32_t key = lv_event_get_key(e);
  if (key == LV_KEY_ESC || key == LV_KEY_BACKSPACE) show_solver_menu();
  else if (key == 'M') { cleanup_solver_ui(); main_menu_create(); }
}
